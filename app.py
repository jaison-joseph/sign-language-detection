...
Author(s): Jaison Joseph George (jaisonj2001@gmail.com)

the file to add logic and run the GUI

This file is essentially a Flask application that allows a user to load and interact with a Support Vector Machine trained to 
detect alphabets of the American Sign Language. The SVM must be one generated by LIBSVM.

Allowing the Flask application to display a video feed was inspired by: https://github.com/hemanth-nag/Camera_Flask_App

Please configure the constants defined after the imports in this file to run the application 
'''
...
'''
we create a Countdown object whose work method is used as the work method of a thread
We keep re creting a thread object every time we need to do a countdown since we can't re use a thread once it's done executing
'''

# we want a 3 second countdown
...
# if we are unable to write recordings to a file, the program will dump to console instead
...
class Store:
    ...
    def storeRecord(self, x, y):
        if self.storeIdx_ == self.storeSize_:
            print('Couldn\'t store record; record is full')
            return
        if x.shape != self.recordShape_:
            print(f'Couldn\'t store record; expected shape {self.recordShape_}, got {x.shape} instead.')
            return
    ...
    '''
    the method to save recorded training sampels, if any
    the save occurs in the following format:

    <folder name>/
        A/
            A<M_D_Y> <H-M-S>.txt (<M_D_Y> <H-M-S> is the current timestamp as per the datetime.now() object) 
        B/
            B<M_D_Y> <H-M-S>.txt
    ...
    note that the directory with the name <folder name> and its subdirectories will be created automatically if they don't exist
    ...
    def saveStore(self):
        ...
        # first check to see if the sub directory to save exists. If not, create it
        folderPath = './' + FOLDER_NAME_
        ...
        for alphabet in set(self.labels_):
            ...
            # now make a folder within the sub directory for each alphabet
            folderPath = './' + FOLDER_NAME_ + '/' + alphabet
            if not os.path.exists(folderPath):
                os.mkdir(folderPath)
            
            # file name format: <ALPHABET> <M_D_Y> <H-M-S>.txt
            ...
            try:
                ...
                print(f"Permission denied: unable to write to file {folderPath + '/' + fileName}. Dumping to console instead")
                print(f"Contents of recordings of alphabet/class {alphabet}:")
                print(np.concatenate([self.store_[i] for i in idxs]))
...
# loading the SVM
try:
    m = None if len(MODEL_NAME_) == 0 else svm_load_model(MODEL_NAME_)
except Exception as e:
    print("There was an error loading the model")
    exit()

store = Store(STORE_SIZE_, [SAMPLE_COUNT_, 63])

# uppercase alphabets representing the labels of the sets of training samples collected
one_recording = np.zeros((SAMPLE_COUNT_, 63))
current_label = ''
frameIdx = 0
...
def gen_frames():  # generate frame by frame from camera
    global out, capture_features,rec_frame, m, store, storeIdx, frameIdx, labels, countdown, countdown_thread, SAMPLE_COUNT_
    ...
    while True:
        ...
        if capture_features:
            if m is None:
                ...
                if countdown_thread.is_alive():
                    ...
                    # if the below condition is true, that means that the recording is occuring (provided that the countdown thred isn't alive)
                    if frameIdx < SAMPLE_COUNT_:
                        ...
                        frameIdx += 1
                    # if capture_features == True & couhtdown thread == False & frameIdx == SAMPLE_COUNT_, then we just finished capturing samples 
                    else:
                        capture_features = False
                        store.storeRecord(one_recording, current_label)
                        frameIdx = 0
                        print(f'done capturing samples for {current_label}')
            else:
                for i, j in idxs.items():
                    features[j[0]] = hand_landmarks.landmark[i].x
                    features[j[1]] = hand_landmarks.landmark[i].y
        ...
        elif toggle_prediction and m is not None:
            for i, j in idxs.items():
                features[j[0]] = hand_landmarks.landmark[i].x
                features[j[1]] = hand_landmarks.landmark[i].y
                features[j[2]] = hand_landmarks.landmark[i].z
...
'''
In each of the routes below, one of the arguments to the render_template method is parts: 
    it specifies what parts of the HTML page are to be shown
        - video: whether to show the frames of the camera captured and processed by the gen_frames() method
        - alphabet: this is a small HTML form to collect the alphabet that will be recorded by the application, should
                    the user choose to record samples 
'''
...
def index():
    ...
    if m is None:
        return render_template(
            'index.html', parts = {"video": True, "alphabet": False}, 
            msg = 'No model loaded. The toggle_alphabet_prediction button will be disabled.'
        )
...
def actions():
    ...
    # if none of the choices are a match, then we have an invalid request, so a 404 page is sent
    return render_template('404.html')

# this is the route used when the user completes the form that accepts an alphabet to record samples
@app.route('/alphabet', methods = ['POST'])
def alphabet():
    global current_label, capture_features, countdown_thread
    action = request.form.get('action')
    if action == 'Submit':
        # retreiving 'alphabet' from the form and seeing if it actually is an alphabet
        ...
        # once we have the alphabet, we can start the countdown thread 
        current_label = char
        countdown_thread = Thread(target=countdown.work)
